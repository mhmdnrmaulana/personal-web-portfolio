...
export async function Contributions() {
  ...
  return (
    <div className="mb-8">
      ...
      <a href={`https://github.com/${METADATA.githubUsername}`} target="_blank" aria-label="View On Github" className="text-blue-600 dark:text-blue-300 flex gap-1 items-center hover:opacity-70 duration-100">
        View On Github
      ...
      <div className="my-3 grid grid-cols-2 grid-rows-2 md:grid-cols-4 md:grid-rows-1 justify-around gap-3">
        <div className="w-full p-3 rounded-md border border-stone-300 dark:border-neutral-800">
          <span>Total</span>
          <p className="text-green-600 text-2xl font-semibold">{contributionsData.total}</p>
        </div>
        <div className="w-full p-3 rounded-md border border-stone-300 dark:border-neutral-800">
          <span>This Month</span>
          <p className="text-green-600 text-2xl font-semibold">{contributionsData.thisMonth}</p>
        </div>
        <div className="w-full p-3 rounded-md border border-stone-300 dark:border-neutral-800">
          <span>This week</span>
          <p className="text-green-600 text-2xl font-semibold">{contributionsData.thisWeek}</p>
        </div>
      </div>
      <div className="w-full h-full pb-5 overflow-x-auto">
        <div className="p-5 min-w-[640px] rounded-md border border-stone-300 dark:border-neutral-800">
          ...
          <div className="flex text-xs md:text-base">
            {data &&
              data.user.contributionsCollection.contributionCalendar.months.map((month: any, index: number) => {
                return (
          ...
          <div className="flex gap-px md:gap-[3px]">
            {data &&
              data.user.contributionsCollection.contributionCalendar.weeks.slice(1).map((week: any, index: number) => (
                <ul key={index} className="w-full flex flex-col gap-px md:gap-1">
                  {week.contributionDays.map((contributionDay: any, index_: number) => {
                    ...
                    return <Square color={color} key={index_} />;
                  })}
                </ul>
              ))}
          </div>
